...
画面サイズはブラウザのバーの出入りだけでは再計算せず、横幅・向きが変わった場合に更新します。標準背面カメラを機器名から明確に識別できる場合はそのレンズを指定し、自動マクロ切替の影響を抑えます。識別できない端末では通常の背面カメラを使います。光学的なピント変化・手ぶれ・認識誤差すべてを除去する保証はありません。追従設定A（filterBeta: 10）は維持しています。
