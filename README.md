...
`target.jpg` と `poster.jpg` は、指定された2本目のサンプル動画の冒頭フレームから作成しています。まず `target.jpg` を印刷するか別端末に表示し、公開URLのカメラから映すと動作を確認できます。
...
2. サイトの「カメラを起動する」を押します。
3. カメラの使用を許可します。
4. `target.jpg` の全体がガイド枠に入るように映します。
5. 認識すると、写真部分に動画が重なります。

同じ端末だけで確認したい場合は、トップ画面の「動画だけを見る」で動画ファイルの再生を先に確認できます。AR認識の確認には、印刷物または別の画面が必要です。
...
2. 「カメラを起動する」を押します。
3. カメラの利用を許可します。
4. 拒否した場合は、iPhoneの設定またはSafariのWebサイト設定から、そのサイトのカメラ権限を「許可」に戻して再読み込みします。
...
2. 「カメラを起動する」を押します。
3. カメラの利用を許可します。
4. 拒否した場合は、Chromeのサイト設定からカメラ権限を許可して再読み込みします。
