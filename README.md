...
登録した画像をスマートフォンのカメラで認識し、その位置・傾き・遠近に追従して動画を重ねるWebARサイトです。チェキに限らず、印刷物や別の画面に表示した画像を利用できます。専用アプリは不要です。
...
現在は指定された縦長の2人の人物画像を `target.jpg` / `poster.jpg` に使用し、動画は「コンポ 1_5.mp4」を配置しています。動画の最初のフレームを自動でサムネイルにしているわけではありません。まず `target.jpg` を印刷するか別端末に表示し、公開URLのカメラから映すと動作を確認できます。
...
- MindARによる画像認識
- 画像に追従する動画表示
- 初回認識でミュート自動再生（端末に拒否された場合だけ操作案内）
- 一瞬の認識切れでは巻き戻さず、250ms以上見失った場合だけ停止・先頭へ戻す
- 再生終了時に1秒かけてフェードアウト。画像を出し入れすると再生
...
- 720p程度
- 5〜15MB程度
- 現在はループせず、最後のフレームを1秒でフェードアウト
- 印刷する静止画と同じ構図から始まる動画

初回はミュートで自動再生を試みます。音声切替ボタンはありません。iPhoneの低電力モードなどによりミュートでも再生が拒否された場合に限り、「動画を再生する」ボタンを表示します。通常時に再生ボタンを出す仕様ではありません。自動再生の制限をWebサイトから解除することはできません。
...
現在の素材は縦長なので、`height` は約 `1.3481` です。素材の縦横比を変更した場合は、原則として `height = 画像の高さ ÷ 画像の幅` にします。

チェキの白枠を含む画像全体を認識対象にする場合は、動画が写真部分だけに重なるよう `width`、`height`、`positionY` を実機で微調整してください。

## ARランタイムを作り直す

MindAR / Three.jsや `tools/stable-mindar.js` / `app/ar-camera-utils.mjs` を変更した場合は、同梱ランタイムを再生成します。

```bash
pnpm build:ar-runtime
```

`public/runtime/mindar-runtime.iife.js` が更新されます。`app/ar-config.ts` の `runtimeFile` の `?v=` も更新してください（配信キャッシュ対策）。通常の素材差し替えでは、この作業は不要です。MindAR 1.2.5のresizeリスナーを停止時に解除できるよう、ビルド時に限定的なパッチを当てています。依存ライブラリ更新時に対象コードが変わればビルドを停止します。
...
ビルド番号、認識状態、実際の動画状態・再生時刻・readyState・最終エラー、カメラ名・入力サイズ・表示サイズ・レイアウト変更回数を表示します。「playing」は認識結果ではなく、動画の再生時刻が進んだ場合にだけ表示します。カメラ機器IDや映像を外部に送信しません。通常アクセスでは表示されません。

## 再生・カメラ修正の検証

`pnpm test:unit` は再生制御とカメラ選択・サイズ固定の動作テスト、`pnpm test` は静的ビルドと全テストです。ブラウザでの実動画・WebGLテクスチャ検証は、プロジェクトルートをローカルHTTPサーバーで配信し `tests/browser/playback.html` を開きます。`tests/browser/camera.html` ではテスト画像から作った仮想カメラ映像を実際のMindARへ入力して認識を確認します。物理カメラは開きません。これらの検証ページは公開ビルドには含めません。

公開ビルドそのものの確認は、ビルド後に `node tests/browser/serve-app.mjs` を実行し、`http://127.0.0.1:8773/?debug=true` を開きます。仮想カメラを注入するローカル専用サーバーです。`&block-autoplay=1` を追加すると自動再生拒否、`&deny-camera=1` を追加するとカメラ許可拒否を再現できます。実際の省電力モードの完全な再現ではなく、拒否時に正しい案内・再試行が行われることの検証です。このサーバーを本番公開に使用しないでください。

PCのテストではiPhone実機のカメラや省電力制限を再現できません。実機では、低電力モードOFF / ONそれぞれで初回認識・短い認識切れ・終了後の再認識・タブ復帰・端末回転を確認してください。低電力モードONで `NotAllowedError` が出た場合、一度のタップが必要なのはブラウザの制約です。[WebKitの報告](https://bugs.webkit.org/show_bug.cgi?id=216887)

画面サイズはブラウザのバーの出入りだけでは再計算せず、横幅・向きが変わった場合に更新します。標準背面カメラを機器名から明確に識別できる場合はそのレンズを指定し、自動マクロ切替の影響を抑えます。識別できない端末では通常の背面カメラを使います。光学的なピント変化・手ぶれ・認識誤差すべてを除去する保証はありません。

現在は利用者が確認・採用した高レスポンス設定（`filterBeta: 100`）です。今後の修正でも、明示的な依頼がない限りこの設定を維持します。Aの `10` から、動いているときの平滑化を弱めて追従遅延を減らしています。認識精度自体が上がる設定や、ライブラリの数値上限ではありません。細かな揺れが増える可能性があります。Aに戻す場合は `app/ar-config.ts` の `filterBeta` だけを `10` に戻し、対応するテストとビルド番号を更新します。再生制御・カメラ・認識開始/解除の猶予は変更していません。
...
- iPhoneの低電力モードをOFFにして再読み込みするか、再生制限の案内が出た場合だけボタンを押します。
- `?debug=true` の `lastError` と `currentTime` を確認します。`NotAllowedError` は自動再生制限、`MediaError:2` はネットワーク、`PlaybackTimeout` は再生時刻が12秒進まなかった状態です。

### 音声が出ない

現在は自動再生を優先し、常にミュートです。音声ON / OFFボタンはありません。
