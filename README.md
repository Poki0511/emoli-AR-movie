...
- 再生終了時に1秒かけてフェードアウト。カードを出し入れすると再生
...
現在は高レスポンス実験版（`filterBeta: 100`）です。Aの `10` から、動いているときの平滑化を弱めて追従遅延を減らしています。認識精度自体が上がる設定や、ライブラリの数値上限ではありません。細かな揺れが増える可能性があります。Aに戻す場合は `app/ar-config.ts` の `filterBeta` だけを `10` に戻し、対応するテストとビルド番号を更新します。再生制御・カメラ・認識開始/解除の猶予は変更していません。
